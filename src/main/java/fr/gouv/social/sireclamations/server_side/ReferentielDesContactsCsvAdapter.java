...
public class ReferentielDesContactsCsvAdapter implements ReferentielDesContacts {
  ...
  public ReferentielDesContactsCsvAdapter(
      @Value("${referentiel.autorite.contact}") Resource csvResource) {
  ...
  @Override
  public List<String> recupererContacts(
      Integer codePostal, Set<AutoriteCompetente> autoriteCompetente) {
    ...
    for (AutoriteCompetente autorite : autoriteCompetente) {
      String nomDeLaColonneDeContactParAutoriteRecherchee =
          "contacts" + autorite.name().toUpperCase();
      if (contactsParAutorite.containsKey(nomDeLaColonneDeContactParAutoriteRecherchee)) {
        emails.addAll(
            Arrays.asList(
                contactsParAutorite
                    .get(nomDeLaColonneDeContactParAutoriteRecherchee)
                    .split("\\|")));
      }
    }
