...
public class ReferentielDesTypeDeMisEnCauseCsvAdapter implements ReferentielDesTypeDeMisEnCause {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(ReferentielDesTypeDeMisEnCauseCsvAdapter.class);
  private final Map<String, CodeTypeDuMisEnCause> libelleToCodeTypeMEC = new HashMap<>();

  public ReferentielDesTypeDeMisEnCauseCsvAdapter(
      @Value("${referentiel.formulaire.type-mec}") Resource csvResource) {
    ...
    for (int i = 1; i < lignes.size(); i++) {
      ...
      if (colonnes.length >= 3 && !colonnes[0].trim().isEmpty() && !colonnes[2].trim().isEmpty()) {
        ...
        try {
          ...
          logger.warn(
              "Le referentiel ne prend actuellement pas en charge le type de mis en cause avec pour libellé : {}",
              libelle);
        }
      }
    }
