...
public class ReferentielDesChampsDuFormulaire {
  private static final Logger logger =
      LoggerFactory.getLogger(ReferentielDesChampsDuFormulaire.class);
  private final Map<ChampsArbreDeDecision, String> champsPourArbreDeDecision =
      new EnumMap<>(ChampsArbreDeDecision.class);

  public ReferentielDesChampsDuFormulaire(
      @Value("${referentiel.formulaire.champs}") Resource csvResource) {
    ...
    for (int i = 1; i < lignes.size(); i++) {
      ...
      if (proprieteArbreDecision != null && !proprieteArbreDecision.isEmpty()) {
        try {
          ...
          logger.error(
              "La valeur '{}' ne correspond à aucun ChampsArbreDeDecision valide",
              proprieteArbreDecision);
        }
      }
    }

    logger.info(
        "Référentiel des champs initialisé avec {} entrées.", champsPourArbreDeDecision.size());
  }
